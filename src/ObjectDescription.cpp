...
#include <opencv2/nonfree/features2d.hpp>   //opencv库函数2d的头文件

#include <boost/thread.hpp>
#include <boost/bind.hpp>
#include <pcl/point_types.h>     //点云的数据类型
#include <pcl/features/fpfh.h>    //点云的特征描述子
#include <pcl/features/normal_3d.h>   //点云的法线  用于计算描述子
#include <pcl/filters/voxel_grid.h>   //点云滤波的头文件
...
ObjectDescription::ObjectDescription(const std::string& pathToVocabs) {
	...
	setHOGVocabPath(pathToVocabs + "/HOGVocab.dat");    //主要是为了获得描述子词袋的位置
...
void ObjectDescription::extractFPFHDescriptors(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud) {
	...
	*cloudPtr = *cloud;
	...
	pcl::VoxelGrid<pcl::PointXYZRGB> grid;  //体素下采样
	...
	*sampledCloudPtr = objectPtCloudSampled;
	...
	pcl::NormalEstimation<pcl::PointXYZRGB, pcl::Normal> ne;  //计算采样后的法线
	...
	ne.setRadiusSearch (0.03);  //计算法线时的半径
	...
	fpfhDescriptors = cv::Mat(ptFeatHistograms->size(), FPFH_LEN, CV_32F);  //fpfh描述子是mat型  FPFH_LEN有定义33  且为float
...
void ObjectDescription::extractSIFTDescriptors(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud) {
	...
	OCLUtils::pointCloudToIntensityImage(*cloud, tempImg);  //从点云数据提取RGB图像进行处理
	cv::resize(tempImg, srcImg, cv::Size(tempImg.cols * zoomFactor, tempImg.rows * zoomFactor)); //对获得的图像进行尺寸，根据相机的数据
	cv::cvtColor(srcImg, srcImgGray, CV_BGR2GRAY);//把RGB转灰度图
	detector.detect(srcImgGray, keypoints);   //检测特征点
	extractor.compute(srcImgGray, keypoints, siftDescriptors);//计算特征点的描素子
